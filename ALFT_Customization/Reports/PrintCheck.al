report 50400 "Print Cheque"
{
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            column(PayeeName; GenJnlLine.Description)
            {
            }
            column(AccVis; AccPay)
            { }
            column(vis1; PAN)
            { }
            column(OtherBanks; OtherBanks)
            { }
            column(PanBanks; PanBanks)
            { }
            ...
            column(ac_payeeonly; TextAC)
            {
            }
            column(ac_payeeonly1; TextAC1)
            {
            }
            column(DateVis; DateVis)
            {
            }
            column(Doc; "Document No.")
            {
            }
            column(CompanyInfoName; companyName)
            {

            }
            column(VendName; UPPERCASE(CompanyInfo.Name))
            {
            }
            column(Add; CompanyInfo.Address)
            {
            }
            column(Add2; CompanyInfo."Address 2")
            {
            }
            column(City; CompanyInfo.City)
            {
            }
            column(PH; CompanyInfo."Phone No.")
            {
            }
            column(BranchNo; BankAcc2."Bank Branch No.")
            {
            }
            column(BalAcc; BankAcc2.Name)
            {
            }
            column(ExtNo; BatchExtDocNo)
            {
            }
            ...
            column(VendCity; Vend.City)
            {
            }
            column(PayeeName1; UPPERCASE(GenJnlLine.Description))
            {
            }
            column(not_negotiable; TextNG)
            {
            ...
            trigger OnAfterGetRecord()
            var
                GenJourLoopRec: Record "Gen. Journal Line";
            begin
                IF GenJurLoop <> 1 THEN
                    CurrReport.SKIP;
                ...
                IF AccPay THEN begin
                    TextAC := 'A/C PAYEE ONLY';
                    TextAC1 := 'A/C PAYEE ONLY';
                    TextNG := 'NOT NEGOTIABLE';
                end
                ELSE begin
                    TextAC := '';
                    TextAC1 := '';
                ...
                GenJourLoopRec.RESET;
                GenJourLoopRec.SETRANGE("Journal Template Name", "Journal Template Name");
                GenJourLoopRec.SETRANGE("Journal Batch Name", "Journal Batch Name");
                GenJourLoopRec.SETRANGE("Document No.", "Document No.");
                GenJourLoopRec.CALCSUMS("Credit Amount");
                TotalNetAmount := GenJourLoopRec."Credit Amount";

                TotalNetAmount := ABS(Amount); //RJ

                RPTVCheck.InitTextVariable;
                RPTVCheck.FormatNoTextCustomized(NumberText, TotalNetAmount, '');
                BatchExtDocNo := '';
                IF GenJourLoopRec.FINDFIRST THEN
                    BatchExtDocNo := GenJourLoopRec."External Document No.";

                //Vend.GET(GenJnlLine."Account No.");
                /*IF "Account Type" = "Account Type"::Vendor THEN
                  IF "Payee Name" = '' THEN
                    PayeeNameReport := Description
                  ELSE
                    PayeeNameReport := "Payee Name"
                ELSE
                  PayeeNameReport := "Payee Name";*/
                GenJurLoop := GenJurLoop + 1;
    ...
    var
        ...
        NumberText: array[2] of Text[130];
