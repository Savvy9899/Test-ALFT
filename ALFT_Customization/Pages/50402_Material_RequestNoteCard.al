page 50402 MaterialRequestNotesCard
{
    Caption = 'Material Request Notes Card';
    PageType = Document;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Prod. Ord. Description"; "Prod. Ord. Description")
                {
                    ApplicationArea = All;
                    Caption = 'FG Item Description';
                    Editable = false;
                }
                field("Prod. Ord. Line No."; "Prod. Ord. Line No.")
                {
                    ApplicationArea = All;
                }
                field(Status; Status)
                ...
                action("Load MRN Lines")
                {
                    ...
                    begin
                        ...
                        if transLine.FindFirst() then
                            Error('Transfer lines already exists');

                        prodOrderComp.Reset();
                        prodOrderComp.SetRange("Prod. Order No.", "Prod. Ord. No.");
                        prodOrderComp.SetRange("Prod. Order Line No.", "Prod. Ord. Line No.");
                        prodOrderComp.SetRange("Approve Technical Verification", true);
                        prodOrderComp.SetRange("Approve Cost Impact", true);
                        prodOrderComp.SetRange("COO Approval", true);
                        if prodOrderComp.FindSet() then begin
                            repeat
                                ...
                                transLine.Validate("Item No.", prodOrderComp."Item No.");
                                transLine.Validate(Description, prodOrderComp.Description);
                                transLine.Validate(Quantity, prodOrderComp."Expected Quantity");
                                item.Get(prodOrderComp."Item No.");
                                item.CalcFields(Inventory);
                                transLine."Current Inventory" := item.Inventory;
                                transLine."Prod. Ord. No." := "Prod. Ord. No.";
                                transLine.Insert(true);
                            until prodOrderComp.Next() = 0;
                            Synchronized := true;
                            Message('Component Lines Inserted Successfully');
                        end
                        else begin
        ...
        area(processing)
        {
            ...
            action("Material Return Note")
            {
                ...
                begin
                    ...
                    Report.RunModal(50413, true, true, transHead);
                end;
            }
            action("Material Requisition & Issues Note")
            {
                ApplicationArea = All;
                Image = Report;
                Promoted = true;
                ...
                begin
                    transHead.Reset();
                    transHead.SetRange("No.", "No.");
                    Report.RunModal(50414, true, true, transHead);
                end;
            }
            action(MRN)
            {
                ...
                begin
                    transHead.Reset();
                    transHead.SetRange("No.", "No.");
                    Report.RunModal(50419, true, true, transHead);
                end;
            }
    ...
    begin
        "Transfer Order Type" := "Transfer Order Type"::MRN;
    end;
